...
import type { OpcoesParse, ResultadoParse, TurnosMes } from '../src/tipos';

const testDir = dirname(fileURLToPath(import.meta.url));
const fixturePath = join(testDir, 'fixtures', 'Escala-SOC-Controle-Agosto.xls');

const baseOpts: OpcoesParse = {
  equipeId: 'soc',
  competencia: '2026-08',
...
async function parseFixture(opts: OpcoesParse = baseOpts): Promise<ResultadoParse> {
  return parsePlanilhaEscala(await fixtureArrayBuffer(), opts);
}

function doc(result: ResultadoParse, login: string): TurnosMes {
  const found = result.documentos.find((documento) => documento.login === login);
  if (!found) throw new Error(`Documento ${login} nao encontrado.`);
  return found;
}
...
function cellText(ws: XLSX.WorkSheet, row: number, col: number): string {
  ...
  const cell: unknown = ws[address];
  if (typeof cell !== 'object' || cell === null) return '';
  const value = (cell as Record<string, unknown>).v;
  return value === undefined ? '' : String(value).trim();
}

function locateCell(ws: XLSX.WorkSheet, wanted: string): { row: number; col: number } {
  const ref = ws['!ref'];
  if (!ref) throw new Error('Worksheet sem !ref.');
  const range = XLSX.utils.decode_range(ref);
  for (let row = range.s.r; row <= range.e.r; row += 1) {
    for (let col = range.s.c; col <= range.e.c; col += 1) {
      if (cellText(ws, row, col) === wanted) return { row, col };
    }
  }
  throw new Error(`Celula ${wanted} nao encontrada.`);
}

function locateTextInColumn(ws: XLSX.WorkSheet, wanted: string, col: number, startRow: number): number {
  const ref = ws['!ref'];
  if (!ref) throw new Error('Worksheet sem !ref.');
  const range = XLSX.utils.decode_range(ref);
  for (let row = startRow; row <= range.e.r; row += 1) {
    if (cellText(ws, row, col) === wanted) return row;
  }
  throw new Error(`Texto ${wanted} nao encontrado na coluna ${col}.`);
}

async function workbookMutado(
...
describe('parsePlanilhaEscala', () => {
  it('parseia a fixture SOC completa conforme contrato', async () => {
    ...
    expect(result.documentos.map((d) => d.login)).not.toEqual(
      expect.arrayContaining(['Domingo', 'Sábado', 'Semana', 'Legenda']),
    );
  ...
  it('reporta valor desconhecido adulterado em memoria', async () => {
    const arquivo = await workbookMutado((wb) => {
      ...
      if (!ws) throw new Error('Escalistas ausente.');
...
describe('assercoes obrigatorias da fixture SOC', () => {
  ...
  it('marca viraDia apenas nos trabalhos do turno Noite', () => {
    expect(['dschlottag', 'luizneto'].every((login) => (
      Object.values(doc(result, login).dias).filter((dia) => dia.seq !== undefined).every((dia) => dia.vd === true)
    ))).toBe(true);
    expect(['aleilima', 'ivcarvalho', 'alamancio', 'altaborda', 'lvergani', 'cestradioto', 'thaisvribeiro'].every((login) => (
      Object.values(doc(result, login).dias).filter((dia) => dia.seq !== undefined).every((dia) => dia.vd === false)
    ))).toBe(true);
  });

  it('ignora resumo de folgas e legenda como documentos', () => {
    expect(result.documentos.map((documento) => documento.login)).not.toEqual(
      expect.arrayContaining(['Domingo', 'Sábado', 'Semana', 'Legenda']),
    );
  });
